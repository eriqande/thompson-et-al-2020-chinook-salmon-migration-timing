...
halflos <- RPGD %>%
  mutate(prince_vec = strsplit(prince, ""),
...
h2 <- halflos %>%
  ...
  mutate(region = str_replace(reg_hap, "[12]", ""),
...
write_phase_input <- function(dirname, D) {
  ...
  str_c("cd ", dirname(outf), "; /Users/eriq/Documents/others_code/PHASE2.1.1/PHASE -S", floor(runif(1, 2, 10000)), " input.txt output 500 1 500 > stdout 2> stderr;")
}
...
summarized <- phase_results %>%
  ...
  unnest(cols = c(freqs)) %>%
  select(-index) %>%
  mutate(distal_region = str_sub(haplotype, 1, 5),
...
hap_freqs <- read_csv("intermediates/011/haplo-freqs-unfiltered.csv") %>%
  mutate(distal_region = str_replace_all(distal_region, "P", "E") %>% str_replace_all(., "M", "L"),
...
tmp <- sf_freqs %>%
  mutate_at(vars(-Pop), function(x) {y <- ifelse(is.na(x), 0, x); sprintf("%.2f", y)}) %>%
