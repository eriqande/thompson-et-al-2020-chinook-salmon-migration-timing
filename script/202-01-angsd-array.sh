...
# we don't enforce a minInd on this one, because we want to get
# all of the sites...
for run_type in fall spring; do
...
angsd -out intermediates/202/MAFS/${run_type}.$IDX \
  ...
  -nind 64 \
  -minInd 30  -minMapQ 30 \
  -SNP_pval 1e-6 > intermediates/202/stdout/${run_type}.$IDX.stdout  2> intermediates/202/stderr/${run_type}.$IDX.stderr
