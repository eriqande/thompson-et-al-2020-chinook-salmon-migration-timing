...
some of the RMarkdown documents requires a proper shell. These were run
on a Mac.
...
Versions we used appear in parentheses.
  ...
  - [`lastz`](http://www.bx.psu.edu/~rsharris/lastz/) (Version 1.04)
    This was obtained and compiled
    thus:
...
This should be updated to reflect where your have stored the jar files
on your system.
...
1.  Download
    ...
    The link provided is for Version 4.3 with a release data of 24
...
might not run on non Unix or Linux architectures.
...
(usually) to the `intermediates/XXX` directories. To facilitate working
between the cluster and a desktop/laptop, some outputs are written to
...
Calculation of allele frequency differences between early and late
running forms across the genome, and around GREB1L, and the generation
of
plots.
...
Use the previously assembled Chinook genome from Narum et al. (2018) to
ascertain variation from that Upper Columbia River summer/spring run
fish around the GREB1L region. The products of this analysis go into
some later trees and haplotype rasters.
...
Use RENT+ (Mirzaei and Wu 2016) to infer the ancestral recombination
...
(2017).  
Data from this RMarkdown document go into Table S9 (“Frequencies of
haplotypes recombinant/non-recombinant between the distal and RoSA
...
determine the likelihood of seeing such a high recombinant frequency in
the Iron Gate Hatchery if spring-fall introgression only occurred after
