...
klaten_nomiss <- rosa_data %>% distinct(Indiv,.keep_all=T) %>%
...
rosa_stats <- rosa_meta %>%
  filter(!str_detect(rosa,"\\?"),
         rosa %in% c("EEEEEEEE","HHHHHHHH","LLLLLLLL")) %>% #remove samples with missing data and recombinant genotypes
...
rosa_stats %>%
  ...
  summarise(mean_jul = mean(julian),
