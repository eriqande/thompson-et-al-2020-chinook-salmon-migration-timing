...
baseline_final %>% count(collection,repunit) %>% arrange(repunit)
```
...
```{r}
mix_est_pb <- infer_mixture(reference = baseline_final, 
...
ggplot(trace_subset, aes(x = repprop, colour = repunit)) +
  geom_density(size = 2) + ylim(0,50)+ theme_bw()
```
...
top_cis <- trace_subset %>%
  group_by(repunit) %>%
  summarise(pt_est = round(mean(repprop),3),
