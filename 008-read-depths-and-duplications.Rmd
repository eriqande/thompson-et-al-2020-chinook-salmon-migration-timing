...
```{r}
dr_bh <- read_rds(file = "stored_results/004/big_haps2.rds") %>%
...
ggplot(NumS_alle_by_indiv, aes(x = fracS_alleles, fill = ecotype)) +
  ...
  facet_wrap(~ecotype) +
  expand_limits(x = 0) +
  scale_fill_manual(values = fcolors_all_sf[c("Fall", "Late_Fall", "Spring", "Winter")]) +
  theme_bw()
```
...
2023 update: we need `indy_pred_by_spring` for another paper.  So, we will
write that out for use:
```{r}
write_rds(indy_pred_by_spring, file = "outputs/008/THOMPSON-2020-indy_pred_by_spring.rds", compress = "xz")
```
